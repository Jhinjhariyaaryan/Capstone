...
// ✅ Unified Exam Submission & Auto-Grading Route
router.post('/submit', async (req, res) => {
  try {
    const { userId, examId, userAnswers, tabSwitches } = req.body;
    ...
    for (let q of exam.questions) {
      const given = Array.isArray(userAnswers)
        ? userAnswers.find(a => a.questionId?.toString() === q._id.toString())
        : null;

      const userAnsText = given
        ? given.answer?.trim()
        : (typeof userAnswers === 'object' ? userAnswers[q._id] : null);
      ...
      if (userAnsText) {
        ...
        if (userAnsText.toLowerCase() === q.correctAnswer.toLowerCase()) {
          ...
          processedAnswers.push({ questionId: q._id, givenAnswer: userAnsText, isCorrect: true, marksObtained: q.marks || 1 });
        } else {
          incorrect++;
          const penalty = exam.negativeMarking ? (exam.negativeMarkPenalty || 0.25) : 0;
          totalScore -= penalty;
          processedAnswers.push({ questionId: q._id, givenAnswer: userAnsText, isCorrect: false, marksObtained: -penalty });
        }
      } else {
    ...
    const percentageScore = Math.max(0, Math.round((totalScore / (totalQuestions * 1)) * 100));
    ...
    const submission = new Submission({
      ...
      unattemptedCount: totalQuestions - attempted,
...
router.get('/dashboard-stats/:userId', async (req, res) => {
  try {
    ...
    const completedCount = await Submission.countDocuments({ $or: [{ student: userId }, { userId: userId }] });
    const totalExams = await Exam.countDocuments();
    const upcomingCount = Math.max(0, totalExams - completedCount);

    res.json({ upcomingCount, progressCount: 0, completedCount });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.get('/recent/:userId', async (req, res) => {
  try {
    const userId = req.params.userId;
    const submissions = await Submission.find({ $or: [{ student: userId }, { userId: userId }] })
...
router.get('/solutions/:submissionId', async (req, res) => {
  try {
    ...
    const solutionData = submission.answers.map((ans, idx) => ({
      index: idx + 1,
      questionText: ans.questionId?.questionText || 'Question removed',
      givenAnswer: ans.givenAnswer,
      correctAnswer: ans.questionId?.correctAnswer || 'N/A',
      explanation: ans.questionId?.explanation || 'No detailed explanation provided.'
    }));

    res.json({
      examTitle: submission.exam?.title || 'Exam Solutions',
      solutions: solutionData
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.get('/certificate/:submissionId', async (req, res) => {
  try {
    ...
    doc.fontSize(16).fillColor('#333').text('This is to certify that', { align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(24).fillColor('#000').text(submission.student?.name || 'Student', { align: 'center' });
    doc.moveDown(0.5);
    doc.fontSize(16).fillColor('#333').text(`has successfully passed the examination for`, { align: 'center' });
    doc.fontSize(20).fillColor('#16a34a').text(submission.exam?.title || 'Exam', { align: 'center' });
    doc.moveDown();
    doc.fontSize(14).text(`Score: ${submission.percentageScore}%`, { align: 'center' });
